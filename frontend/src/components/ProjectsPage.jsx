...
const ProjectsPage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const fundedProjects = projects.filter(
          (project) => parseFloat(project.current_funding) > 0
        ).length;
        const funding = projects.reduce(
          (acc, project) => acc + parseFloat(project.current_funding),
          0
        );
  ...
  return (
    <div className="body">
      ...
      <h4>
        Explore{" "}
        <span style={{ color: "rgb(0, 158, 116)" }}>
      ...
      <div className="card-container--2">
        {projects.map((project) => {
          ...
          if (
            project.remaining_time !== "Due date ended" &&
